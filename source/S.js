...
S.TryPlayingChannel = function (ch) {
  if (Host.realtime - (ch.play_failed_time || 0) < 3) {
    return;
  }

  ch.audio.play().catch((e) => {
    Con.Print(`S.TryPlayingChannel: failed to play audio, ${e.message}, retrying at a later point`);
    ch.play_failed_time = Host.realtime;
  }).then(() => {
    ch.play_failed_time = null;
  });
}
...
S.Init = function() {
  ...
  Cmd.AddCommand('soundlist', S.SoundList_f);
  ...
  let i; const ambient_sfx = ['water1', 'wind2']; let ch; let nodes;
...
S.Spatialize = function(ch) {
  ...
  dist *= ch.dist_mult;
  const dot = S.listener_right[0] * source[0] +
...
S.StartSound = function(entnum, entchannel, sfx, origin, vol, attenuation) {
  ...
  const target_chan = S.PickChannel(entnum, entchannel);
  ...
  sfx.NotifyDataAvailable = (sc) => {
    target_chan.sfx = sfx;
    target_chan.pos = 0.0;
    target_chan.end = Host.realtime + sc.length;
    let volume;
    if (S.context != null) {
      const nodes = {
        source: S.context.createBufferSource(),
      ...
      for (i = 0; i < S.channels.length; ++i) {
        ...
        try {
          nodes.source.stop(nodes.source.buffer.length);
        } catch (e) {
        }
        break;
      }
...
S.SoundList_f = function() {
  let total = 0; let i; let sfx; let sc; let size;
  for (i = 0; i < S.known_sfx.length; ++i) {
    sfx = S.known_sfx[i];
    sc = sfx.cache;
    if (sc == null) {
...
S.UpdateAmbientSounds = function() {
  ...
  let i; let ch; let vol; let sc;

  const l = Mod.PointInLeaf(S.listener_origin, CL.state.worldmodel);
...
S.UpdateDynamicSounds = function() {
  let i; let ch; let sc; let volume;
  for (i = 0; i < S.channels.length; ++i) {
    ch = S.channels[i];
    if (ch == null) {
      continue;
...
S.UpdateStaticSounds = function() {
  let i; let j; let ch; let ch2; let sfx; let sc; let volume;
...
S.LoadSound = function(s) {
  ...
  let p; let fmt; let dataofs; let datalen; let cue; let loopstart; let samples;
  ...
  if (loopstart != null) {
    sc.loopstart = loopstart * fmt.blockAlign / fmt.samplesPerSec; // FIXME: duration vs samples
  }
  if (samples != null) {
  ...
  if (S.context != null) {
    sc.data = null;
    S.context.decodeAudioData(out).then((audioData) => {
      sc.data = audioData;
