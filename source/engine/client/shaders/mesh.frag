...
// (see ShadowMap.selectPointLights()); combined via min() below since mesh
...
void main(void){
  ...
  vec3 specular = (pow(specDot, kMeshSpecularShininess) * uShadeLight * shadow
                 + pow(dynamicSpecDot, kMeshSpecularShininess) * uDynamicShadeLight * pointShadow) * kMeshSpecularIntensity;
