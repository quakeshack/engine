...
let { CL, COM, Draw, SCR } = getClientRegistry();

eventBus.subscribe('registry.frozen', () => {
  ({ CL, COM, Draw, SCR } = getClientRegistry());
});

export default class Sbar {
  static lines = 0;
  static showscores = false;
  static scoreboardlines = 0;
  static fragsort: number[] = [];

  // Core WAD textures — initialized in Init()
  static nums: GLTexture[][] = null!;
  static colon: GLTexture = null!;
  static slash: GLTexture = null!;
  static weapons: GLTexture[][] = null!;
  static ammo: GLTexture[] = null!;
  static armor: GLTexture[] = null!;
  static items: GLTexture[] = null!;
  static sigil: GLTexture[] = null!;
  static faces: GLTexture[][] = null!;
  static face_invis: GLTexture = null!;
  static face_invuln: GLTexture = null!;
  static face_invis_invuln: GLTexture = null!;
  static face_quad: GLTexture = null!;
  static sbar: GLTexture = null!;
  static ibar: GLTexture = null!;
  static scorebar: GLTexture = null!;
  static ranking: GLTexture = null!;
  static complete: GLTexture = null!;
  static inter: GLTexture = null!;
  static finale: GLTexture = null!;
  static disc: GLTexture = null!;

  // Hipnotic mission pack textures (optional)
  static h_weapons?: GLTexture[][];
  static h_items?: GLTexture[];
  static hipweapons?: number[];

  // Rogue mission pack textures (optional)
  static r_invbar?: GLTexture[];
  static r_weapons?: GLTexture[];
  static r_items?: GLTexture[];
  static r_teambord?: GLTexture;
  static r_ammo?: GLTexture[];

  static ShowScores(): void {
    Sbar.showscores = true;
  }

  static DontShowScores(): void {
  ...
  static async Init(): Promise<void> {
    ...
    const [ranking, complete, inter, finale] = await Promise.all([
      Draw.LoadPicFromLump('ranking'),
      Draw.LoadPicFromLump('complete'),
      Draw.LoadPicFromLump('inter'),
      Draw.LoadPicFromLump('finale'),
    ]);
  ...
  static DrawPic(x: number, y: number, pic: GLTexture): void {
  ...
  static DrawCharacter(x: number, y: number, num: number): void {
    if (CL.state.maxclients > 1) {
  ...
  static DrawString(x: number, y: number, str: string): void {
    if (CL.state.maxclients > 1) {
  ...
  static DrawNum(x: number, y: number, num: number, digits: number, color: number): void {
  ...
  static IntermissionNumber(x: number, y: number, num: number): void {
    let str = num.toString();
    if (str.length > 3) {
  ...
  /** Draws the score board when pressing Tab. */
  static DeathmatchOverlay(): void {
  ...
  /** Draws the mini scoreboard overlay inside the status bar. */
  static MiniDeathmatchOverlay(): void {
    Sbar.SortFrags();
    ...
    for (let k, s, num; (i < l) && (y < (VID.height - 8)); i++) {
      k = Sbar.fragsort[i];
      s = CL.state.scores[k];
      if (s.name.length === 0) {
        continue;
  ...
  static ChatterDrawOverlay(): void {
  ...
  static IntermissionOverlay(): void {
    if (CL.state.maxclients > 1) {
      Sbar.DeathmatchOverlay();
      return;
    }
