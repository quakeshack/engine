...
3. **Document magic numbers** with a comment explaining the derivation. Example:
   ...
   ```
4. **Prefer precise assertions** (`assert.deepEqual`, `assert.equal`) over loose checks.
5. **Use `assertNear`** for any floating-point comparison.
6. **Avoid `Math.random` in production paths** — if production uses it, save and restore in tests:
   ```javascript
