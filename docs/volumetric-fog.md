...
**Color:** The fog color is automatically derived from the average color of the liquid texture. If the color cannot be determined (e.g. no turbulent surface is visible from the camera position), the engine falls back to content-type defaults: orange-red for lava, green for slime, and blue for water.
