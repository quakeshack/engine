...
Used in production today — every Hellwave map compiles with `BRUSHLIST` as of this writing, and
after the latest round of fixes to the brush/hull traceline reconciliation code it holds up well
across normal gameplay. "Experimental" reflects two specific, still-open caveats, not general
instability, and is an engine-level property of the code path below, not something tied to any
one game:
